...
class KeyboardCommandHandler:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "velocity": {"linear": 0.25, "rotation": 0.50},
            "ramp": {"ramp_time_ms": 1000, "update_interval_ms": 50, "ramp_update_interval_ms": 20},
        }
    ...
    def handle_key_release(self, action: str) -> None:
        ...
        if action == "forward" or action == "backward":
            ...
            if (
                "strafe_left" not in self.pressed_actions
                and "strafe_right" not in self.pressed_actions
            ):
                self.set_target_velocity("strafe", 0.0)
        elif action == "rotate_left" or action == "rotate_right":
            if (
                "rotate_left" not in self.pressed_actions
                and "rotate_right" not in self.pressed_actions
            ):
    ...
    def stop_movement(self) -> None:
        ...
        if self.client:
            try:
                ...
                asyncio.run_coroutine_threadsafe(self.client.stop_move(), loop)
            except Exception as e:
