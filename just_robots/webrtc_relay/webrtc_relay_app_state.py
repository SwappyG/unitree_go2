...
from just_robots.firebase.firebase_server import (
    FirebaseAuthConfig,
    FirebaseUser,
...
class WebRTCRelayAppState:
    firebase_auth: FirebaseAuthConfig | None = None
    ...
    client_to_relay_stats_monitor: "WebRTCStatsMonitor | None" = (
        None  # CLIENT→RELAY (from relay's perspective)
    )
    ...
    current_user_id: str | None = None  # Firebase UID of connected user
    current_user_email: str | None = None  # Email of connected user
    last_activity_time: float | None = None  # Timestamp of last activity
    idle_timeout_task: asyncio.Task[None] | None = (
        None  # Background task for idle timeout monitoring
    )
    ...
    async def close_rtc_relay_connection(self):
        ...
        if self.relay_rtc_data_channel is not None:
            ...
            try:
                ...
                logger.warning(
                    f"failed to close existing relay rtc peer data connection. {exception=}"
                )
            finally:
    ...
    async def _force_disconnect_idle_user(self):
        ...
        logger.info(
            f"Successfully disconnected idle user: {disconnected_user} ({disconnected_email})"
        )
...
async def get_user(
    ...
    return await verify_firebase_token(
        credentials=credentials, config=get_app_state(request).firebase_auth
    )
