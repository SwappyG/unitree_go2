...
from just_robots.firebase.firebase_server import FirebaseUser
...
def _on_go2_message(state: WebRTCRelayAppState, robot_data: RobotData):
    ...
    if not state.relay_rtc_data_channel or state.relay_rtc_data_channel.readyState != "open":
        logger.debug(f"got message from go2, but datachannel is not open {robot_data.raw_message=}")
        return
    ...
    try:
        if state.go2 is not None:
            ...
            for topic in topics_to_subscribe_to:
                state.go2.data_channel.send(json.dumps({"type": "subscribe", "topic": topic}))
...
async def _on_go2_video_track(
    state: WebRTCRelayAppState, track: MediaStreamTrack, _robot_num: str | int
):
    """
...
async def connect(
    ...
    user: t.Annotated[FirebaseUser, Depends(get_user)],
):
    """
    ...
    if state.current_user_id is not None:
        if state.current_user_id == user.uid:
            # Same user reconnecting - just update activity and allow
            state.update_activity()
            logger.info(
                f"User {user.uid} ({user.email}) already connected, updating activity timestamp"
            )
            ...
            if state.last_activity_time:
                ...
                logger.info(
                    ...
                    f"after {idle_time:.1f}s of inactivity to allow {user.uid} ({user.email})"
                )
                # Force disconnect the idle user
                await state._force_disconnect_idle_user()  # type: ignore[reportPrivateUsage] # noqa: SLF001
            else:
                # No activity time recorded, disconnect anyway
                logger.warning(
                    f"Disconnecting existing connection without activity time to allow {user.uid}"
                )
    ...
    state.current_user_id = user.uid
    ...
    if go2.pc.connectionState != "connected":
        logger.warning(
            f"GO2 connection state is {go2.pc.connectionState} after {waited:.1f}s, starting stats monitor anyway"
        )
    else:
        logger.info(
            f"GO2 connection established (state: {go2.pc.connectionState}), starting stats monitor"
        )
...
async def disconnect(
    ...
    if state.current_user_id is not None and state.current_user_id != user.uid:
        logger.warning(
            f"User {user.uid} ({user.email}) attempted to disconnect, "
    ...
    logger.info(f"User {user.uid} ({user.email}) disconnecting")
    ...
    logger.info(f"User {user.uid} ({user.email}) successfully disconnected")
...
async def update_subscriptions(
    ...
    state: t.Annotated[WebRTCRelayAppState, Depends(get_app_state)],
    _user: t.Annotated[FirebaseUser, Depends(get_user)],
):
    """
    ...
    def send_subscription(action: str, topic: str):
        if state.go2 is not None:
            state.go2.data_channel.send(json.dumps({"type": action, "topic": topic}))
...
async def get_subscriptions(
    ...
    return {"subscribed_topics": list(state.subscribed_topics)}
